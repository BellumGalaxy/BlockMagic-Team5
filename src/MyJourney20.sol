...
contract MyJourney20 is ERC721URIStorage, AccessControl { //}, FunctionsClient {
    ...
    modifier onlyEducationalInstitution() {
        require(s_educationalInstitutions[msg.sender], "Caller is not an authorized educational institution");
        _;
    }
    ...
    function addStudent(address _studentAddress, uint256 _id, string memory _name) external onlyEducationalInstitution {
        s_students[_studentAddress] = Student(_id, _studentAddress, _name);
        s_is_registered[_studentAddress] = true;
    }

    function removeStudent(address _studentAddress) external onlyEducationalInstitution {
        s_is_registered[_studentAddress] = false;
    }

    function issueNFT(address _studentAddress) external onlyEducationalInstitution {
        require(s_is_registered[_studentAddress], "Student does not exist");
    ...
    function issueNFTForStage(address _studentAddress, EducationStage _stage) external onlyEducationalInstitution {
        require(s_is_registered[_studentAddress], "Student does not exist");
    ...
    function addEducationalInstitution(address _institutionAddress) external onlyAdministrator {
        s_educationalInstitutions[_institutionAddress] = true;
    }

    function removeEducationalInstitution(address _institutionAddress) external onlyAdministrator {
        s_educationalInstitutions[_institutionAddress] = false;
    }
    
    function setNFTMetadataURI(uint256 _tokenId, string memory _tokenURI) external onlyEducationalInstitution {
